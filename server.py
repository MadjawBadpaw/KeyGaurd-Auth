...
app.add_middleware(CORSMiddleware,
    allow_origins=["http://localhost:8000", "http://127.0.0.1:8000"],  # fix: no wildcard
...
class ConfigPayload(BaseModel):
    ...
    contamination:   float = 0.05   # fix: configurable
...
async def index():
    ...
    return JSONResponse({"status": "place index.html in static/"})
...
@app.get("/config")
async def get_config(): return _load_config()
...
async def set_mode(payload: ModePayload):
    if payload.mode not in VALID_MODES:           # fix: validate mode
    ...
    try:
        ...
        cfg = _load_config(); cfg["mode"] = payload.mode; _save_config(cfg)
    except ValueError as e:
        raise HTTPException(400, str(e))
...
async def stop_agent(payload: PasswordPayload):
    ...
    async def _exit():
        ...
        sys.exit(0)                               # fix: sys.exit not os._exit
...
def _load_config():
    if CONFIG_PATH.exists():
        try:
            ...
            logger.log("ERROR", {"detail": f"load_config: {e}"})
...
def _save_config(cfg):
    ...
    try:
        ...
        logger.log("ERROR", {"detail": f"save_config: {e}"})